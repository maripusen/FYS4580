...
for i in range (1,18):
    if i == 69:
        pass
#    if i in [8,9,10]:
#        universii.append([f] * int(assembly_x -3 /2) + [c]*3 + [f] * int(assembly_x -3 /2))
    else:
        universii.append([f]*assembly_x)

#print(universii[7][8])

#universii = universii *150

f_assembly = openmc.RectLattice(name = "fuel_assembly")
f_assembly.lower_left = [-assembly_x*pinx,-assembly_y*piny,-assembly_z]
f_assembly.upper_right = [assembly_x*pinx, assembly_y*piny, assembly_z]
f_assembly.universes = universii
f_assembly.pitch = [pinx*-2,piny*-2, pinz *-2]
...
plot.to_ipython_image(title = "V2")
